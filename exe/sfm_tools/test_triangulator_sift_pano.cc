...
void FeatureExtraction(FeatureDataContainer &feature_data_container,
    ...
    // Panorama Config
    sift_extraction.use_panorama_config = static_cast<bool>(param.GetArgument("use_panorama_config", 0));
    if (sift_extraction.use_panorama_config) {
        std::string panorama_config_path = param.GetArgument("panorama_config_path", "");
        // Load panorama config
        LoadParams(panorama_config_path, sift_extraction.panorama_config_params);
    }
...
int main(int argc, char *argv[]) {
    ...
    if (boost::filesystem::exists(JoinPaths(old_workspace_path, "/local_cameras.bin"))) {
        old_feature_data_container->ReadCamerasBinaryData(JoinPaths(old_workspace_path, "/cameras.bin"), false);
        old_feature_data_container->ReadLocalCamerasBinaryData(JoinPaths(old_workspace_path, "/local_cameras.bin"));
    } else {
        old_feature_data_container->ReadCamerasBinaryData(JoinPaths(old_workspace_path, "/cameras.bin"));
    }
