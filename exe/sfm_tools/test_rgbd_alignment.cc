...
void MapUpdate(const std::shared_ptr<sensemap::FeatureDataContainer> &feature_data_container,
    ...
    for (auto image_id : feature_data_container->GetNewImageIds()) {
        if (fixed_image_ids.find(image_id) != fixed_image_ids.end()){
            continue;
        }
...
void AlignSFM(const std::shared_ptr<sensemap::FeatureDataContainer> &feature_data_container,
                const std::shared_ptr<sensemap::SceneGraphContainer> &scene_graph_container,
                std::shared_ptr<sensemap::ReconstructionManager> &reconstruction_manager, 
                const Reconstruction & target_reconstruction,
                Configurator &param, struct HybridOptions hybrid_options = HybridOptions()) {
    using namespace sensemap;

    PrintHeading1("AlignSFM");
    // trianglulation
    std::shared_ptr<IncrementalMapper> mapper = std::make_shared<IncrementalMapper>(scene_graph_container);
    int rect_idx = reconstruction_manager->Add();
    auto reconstruction = reconstruction_manager->Get(rect_idx);
    mapper->BeginReconstruction(reconstruction);
    ...
    // std::cout << "rec_image_ids: " << rec_image_ids.size() << std::endl;
    
    const auto& target_image_names = target_reconstruction.GetImageNames();

    int64_t triangulated_image_count = 0;
    for(const auto& rec_image : rec_image_ids){
        const image_t image_id = rec_image.first;
        const std::string image_name = rec_image.second.Name();
        auto& image = reconstruction->Image(image_id);

        if (!scene_graph_container->CorrespondenceGraph()->ExistsImage(image_id)) {
            continue;
        }
        if (target_image_names.find(image_name) == target_image_names.end()){
            // std::cout << "no find " << image_name << std::endl;
            continue;
        }

        const auto& target_image_id = target_image_names.at(image_name);
        if (!target_reconstruction.IsImageRegistered(target_image_id)){
            continue;
        }
        const auto& target_image = target_reconstruction.Image(target_image_id);
        image.Qvec() = target_image.Qvec();
        image.Tvec() = target_image.Tvec();
        reconstruction->RegisterImage(image_id);

        auto& camera = reconstruction->Camera(image.CameraId());
        camera = target_reconstruction.Camera(target_image.CameraId());

        PrintHeading2(StringPrintf("Triangulating image #%d - %s (%d / %d)", 
            image_id, image_name.c_str(), triangulated_image_count++, rec_image_ids.size()));
    ...
    std::cout << "tri_images: " << reconstruction->RegisterImageIds().size() << std::endl;
    if (reconstruction->RegisterImageIds().size() < 5){
        reconstruction_manager->Delete(rect_idx);
        IncrementalSFM(scene_graph_container, reconstruction_manager, param, hybrid_options);
        CHECK(reconstruction_manager->Size() > 0);
        return;
    }
    ...
    // Bundle adjustment
    {
        ...
        for(size_t idx = 0; idx < reg_image_ids.size(); idx++){
            const image_t image_id = reg_image_ids.at(idx);
            if (!reconstruction->IsImageRegistered(image_id)){
                continue;
            }
            ba_config.AddImage(image_id);
    ...
    MapUpdate(feature_data_container, scene_graph_container, reconstruction_manager, param, HybridOptions());
    std::cout << "map update images: " << reconstruction->RegisterImageIds().size() << std::endl;
    return;
}
...
std::shared_ptr<Reconstruction> RGBDRegistration(
    const Reconstruction & target_reconstruction,
    Configurator & param, 
    const LinearWeightInterpolator & interp) {
...

    {
        ...
        std::cout << "feature_data_container size: " << feature_data_container->GetImageIds().size() << std::endl;
        AlignSFM(feature_data_container, scene_graph_container, reconstruction_manager, target_reconstruction, param, hybrid_options);
        if (reconstruction_manager->Size() > 0) {
            auto save_path = JoinPaths(workspace_path, "0-alignSfM");
            CreateDirIfNotExists(save_path);
            reconstruction_manager->Get(0)->WriteBinary(save_path);
        }
...
bool AddRgbdReconstruction(
    ...
    for(int i_cam = 0; i_cam < rgbd_infos.size(); i_cam++)
    {
        ...
        for (const auto & r2t : rgbd_2_target){
            std::string name = r2t.first.substr(16);
...
int main(int argc, char *argv[]) {
    ...
    for(int i = 1; i <= num_to_register; i++)
    {
        ...
        else
        {
            ...
            ExceptionHandler(RGBD_CALIB_FAILED, 
                JoinPaths(GetParentDir(workspace_path), "errors/dense"), tool_name).Dump();
    ...
    std::vector<std::string> rgbd_subpaths;
    for (auto & info : rgbd_infos) {
        rgbd_subpaths.emplace_back(info.sub_path);
    }
    ...
    const auto old_image_ids = target_reconstruction.RegisterImageIds();

    auto interp = GetIndexWeightInterpolator(param, target_reconstruction);
    auto align_reconstruction = RGBDRegistration(target_reconstruction, param, interp);
    ...
    bool result = OptimizeDeltaTime(image_path, *align_reconstruction, image_alignment, 
                                                                     map_index_to_image_id, v_map_rgbd_index_to_name, 
                                                                     target_ms_per_frame);

    result = result ? AddRgbdReconstruction(image_path, param, *align_reconstruction, 
                                              feature_data_container, scene_graph_container,
                                              reconstruction_manager, image_alignment, map_index_to_image_id, 
    ...
    if (rescale_flag) {
        for (int i = 0; i < 2; i++) {
            double temp_factor = RescaleByStatitics(image_path, image_alignment, target_reconstruction);
            scale_factor *= temp_factor;
            std::cout << "\t=> temp_factor " << i << ": " << temp_factor << std::endl;
        }
    } else {
