...
int main(int argc, char* argv[]) {
    ...
    const bool use_gps_prior = static_cast<bool>(param.GetArgument("use_gps_prior", 0));
    if (!use_gps_prior){
        std::cout << "use_gps_prior is false" << std::endl;
        return 0;
    }

    const std::string workspace_path = param.GetArgument("workspace_path", "");
    ...
    for (auto id : image_ids){
        ...
        Eigen::Vector3f image_pose((float)image.ProjectionCenter().x(), (float)image.ProjectionCenter().y(), 1.f);
    ...
    // std::vector<PlyPoint> trans_pnts;
    // for (int i = 0; i < image_poses.size(); i++){
    //     Eigen::Vector3f trans_pose = pivot * image_poses.at(i);
    //     PlyPoint pnt;
    //     pnt.x = trans_pose.x();
    //     pnt.y = trans_pose.y();
    //     pnt.z = trans_pose.z();
    //     trans_pnts.push_back(pnt);
    // }
    // WriteTextPlyPoints(rec_path + "/box.ply", trans_pnts, 0, 0);
