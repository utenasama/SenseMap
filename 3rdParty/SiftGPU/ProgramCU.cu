...
void ProgramCU::GenerateList(CuTexImage* list, CuTexImage* hist)
{
	...
	ListGen_Kernel<<<grid, block>>>(listTex.handle, histTex.handle, (int4*) list->_cuData, len, hist->GetImgWidth());
}
...
void __global__ MultiplyDescriptor_Kernel(cudaTextureObject_t texDes1, cudaTextureObject_t texDes2, int* d_result, int num1, int num2, int3* d_temp, int dim)
{
...
void ProgramCU::MultiplyDescriptor(CuTexImage* des1, CuTexImage* des2, CuTexImage* texDot, CuTexImage* texCRT, int dim)
{
...
struct Array2D1x9{float arr[9];};
struct Array2D9x9{float arr[81];};
#define RAD2DEG(rad) (rad) * 57.29577951308232286464772187173366546630859375

void __global__ MultiplyDescriptorG_Kernel_E(cudaTextureObject_t texDes1, cudaTextureObject_t texDes2,
	cudaTextureObject_t texLoc1, cudaTextureObject_t texLoc2, int* d_result, int num1, int num2, int3* d_temp,
	Array2D9x9 Hs, int lengthHomo, float hdistmax, Array2D1x9 E, float fdistmax, int dim)
{
	...
	for(int i = 0; i < MULT_BLOCK_DIMY; ++i)
	{
		...
		if(idx1 + i < num1)
		{
			...
			for (int k = 0; k < lengthHomo; ++k) {
				x[0] = Hs.arr[k * 9] * locx + Hs.arr[k * 9 + 1] * locy + Hs.arr[k * 9 + 2];
				x[1] = Hs.arr[k * 9 + 3] * locx + Hs.arr[k * 9 + 4] * locy + Hs.arr[k * 9 + 5];
				x[2] = Hs.arr[k * 9 + 6] * locx + Hs.arr[k * 9 + 7] * locy + Hs.arr[k * 9 + 8];
			...
			if(check_H)
			{
				...
				float loc2_norm = sqrt(loc2.x * loc2.x + loc2.y * loc2.y + 1.0f);
				float fx1_norm = sqrt((fx1[0] * fx1[0] + fx1[1] * fx1[1] + fx1[2] * fx1[2]));
				float ftx2_norm = sqrt((ftx2[0] * ftx2[0] + ftx2[1] * ftx2[1] + ftx2[2] * ftx2[2]));
...
void __global__ MultiplyDescriptorG_Kernel_F(cudaTextureObject_t texDes1, cudaTextureObject_t texDes2,
	cudaTextureObject_t texLoc1, cudaTextureObject_t texLoc2, int* d_result, int num1, int num2, int3* d_temp,
	Array2D9x9 Hs, int lengthHomo, float hdistmax, Array2D1x9 F, float fdistmax, int dim)
{
	int idx01 = (blockIdx.y  * MULT_BLOCK_DIMY);
	int idx02 = (blockIdx.x  * MULT_BLOCK_DIMX);
...
void ProgramCU::MultiplyDescriptorG(CuTexImage* des1, CuTexImage* des2,
		...
		float* H, int lengthHomo, float hdistmax, float* F, float fdistmax, float* E, int dim)
{
	if (F != NULL) {
		int num1, num2;
		...
		CuTexImage::CuTexObj des1Tex = des1->BindTexture(texDataDesc, cudaCreateChannelDesc<uint4>());
		CuTexImage::CuTexObj des2Tex = des2->BindTexture(texDataDesc, cudaCreateChannelDesc<uint4>());
		MultiplyDescriptorG_Kernel_F<<<grid, block>>>(des1Tex.handle, des2Tex.handle, loc1Tex.handle, loc2Tex.handle,
													(int*)texDot->_cuData, num1, num2,
													(texCRT? (int3*)texCRT->_cuData : NULL),
													MatHs, lengthHomo, hdistmax, MatF, fdistmax, dim);
	} else if (E != NULL) {
		...
		CuTexImage::CuTexObj des1Tex = des1->BindTexture(texDataDesc, cudaCreateChannelDesc<uint4>());
		CuTexImage::CuTexObj des2Tex = des2->BindTexture(texDataDesc, cudaCreateChannelDesc<uint4>());
		MultiplyDescriptorG_Kernel_E<<<grid, block>>>(des1Tex.handle, des2Tex.handle, loc1Tex.handle, loc2Tex.handle,
													(int*)texDot->_cuData, num1, num2,
													(texCRT? (int3*)texCRT->_cuData : NULL),
													MatHs, lengthHomo, hdistmax, MatE, fdistmax, dim);
	}
}
...
void __global__  RowMatch_Kernel(int*d_dot, int* d_result, int num2, float distmax, float ratiomax, int dim)
{
...
void ProgramCU::GetRowMatch(CuTexImage* texDot, CuTexImage* texMatch, float distmax, float ratiomax, int dim)
{
	...
	RowMatch_Kernel<<<grid, block>>>((int*)texDot->_cuData,
		(int*)texMatch->_cuData, num2, distmax, ratiomax, dim);
}
...
void __global__  ColMatch_Kernel(int3*d_crt, int* d_result, int height, int num2, float distmax, float ratiomax, int dim)
{
...
void ProgramCU::GetColMatch(CuTexImage* texCRT, CuTexImage* texMatch, float distmax, float ratiomax, int dim)
{
	...
	ColMatch_Kernel<<<grid, block>>>((int3*)texCRT->_cuData, (int*) texMatch->_cuData, height, num2, distmax, ratiomax, dim);
}
