...
void MyMesh::set_labels(const int* labels) {
    const int nb_vert = this->vert.size();
    vcg::MyMesh::VertexIterator vi = this->vert.begin();
                          ...
                          MyMesh& samples)
{
    ...
    {
        std::cerr << "ERROR poisson disk sampling:" << std::endl;
        std::cerr << "you must specify a maximal radius" << std::endl;
        std::cerr << "or a minimal number of samples" << std::endl;
    }
