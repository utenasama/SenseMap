...
public:
    ...
    void set_labels(const int* labels);
...
public:
    ...
    void AddFace(const MyMesh::FaceType& f, MyMesh::CoordType p)
    {
        ...
        int best_idx = 0;
        double angle_diff0 = f.V(0)->N().dot(mesh->vert.back().N());
        double angle_diff1 = f.V(1)->N().dot(mesh->vert.back().N());
        double angle_diff2 = f.V(2)->N().dot(mesh->vert.back().N());
        double best_angle = angle_diff0;
        if (best_angle > angle_diff1) {
            best_angle = angle_diff1;
            best_idx = 1;
        }
        if (best_angle > angle_diff2) {
            best_angle = angle_diff2;
            best_idx = 2;
        }
        mesh->vert.back().L() = f.V(best_idx)->L();
    }
