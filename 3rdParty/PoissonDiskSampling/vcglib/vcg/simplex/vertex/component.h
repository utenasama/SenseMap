...
template <class T> class Label: public T {
public:
  	int &L() { return _label; }
  	const int &cL() const { return _label; }
	template < class LeftV>
	void ImportData(const LeftV  & left ){
		if(LeftV::HasLabel())
			L() = left.cL();
  	...
  	static bool HasLabel()   { return true; }
	static void Name(std::vector<std::string> & name){name.push_back(std::string("Label"));T::Name(name);}
