...
void ComputeSquaredSampsonError(const Eigen::Vector2d& point1,
  ...
  const double x1_0 = point1(0);
  const double x1_1 = point1(1);
  const double x2_0 = point2(0);
  const double x2_1 = point2(1);
...
void ComputeSquaredSampsonErrorSphericalCamera(
  ...
  for (size_t i = 0; i < points1.size(); ++i) {
    ...
    const double weight1 = 1+points1[i].squaredNorm();
    const double weight2 = 1+points2[i].squaredNorm();
    ...
    (*residuals)[i] =
        x2tEx1 * x2tEx1 /
        (weight1*(Ex1_0 * Ex1_0 + Ex1_1 * Ex1_1) + 
         weight2*(Etx2_0 * Etx2_0 + Etx2_1 * Etx2_1));
  }
}
void ComputeAngularError(const std::vector<Eigen::Vector3d>& points1_normalized,
                         const std::vector<Eigen::Vector3d>& points2_normalized,
                         const Eigen::Matrix3d& E,
                         std::vector<double>* residuals){
    
	CHECK_EQ(points1_normalized.size(), points2_normalized.size());
	residuals->resize(points1_normalized.size());	

	for(size_t i = 0; i<points1_normalized.size(); ++i){
		Eigen::Vector3d point1_h = points1_normalized[i];
		Eigen::Vector3d point2_h = points2_normalized[i];
...
void ComputeSquaredReprojectionErrorSpherical(
    const std::vector<Eigen::Vector3d>& points2D_bearing,
    const std::vector<Eigen::Vector3d>& points3D,
    const Eigen::Matrix3x4d& proj_matrix, std::vector<double>* residuals) {
  CHECK_EQ(points2D_bearing.size(), points3D.size());

  residuals->resize(points2D_bearing.size());
  ...
  for (size_t i = 0; i < points2D_bearing.size(); ++i) {
    const double X_0 = points3D[i](0);
    const double X_1 = points3D[i](1);
    const double X_2 = points3D[i](2);
    ...
    Eigen::Vector3d bearing2D = points2D_bearing[i];
    Eigen::Vector3d bearing3D;
    ...
    (*residuals)[i] = (bearing3D-bearing2D).squaredNorm();
  }
}
