...
#ifndef SENSEMAP_OPTIM_SOLVER_L1_SOLVER_H_
#define SENSEMAP_OPTIM_SOLVER_L1_SOLVER_H_
...
public:
	...
	void Solve(const Eigen::VectorXd& rhs, Eigen::VectorXd* solution) {
		...
		for (int i = 0; i < options_.max_num_iterations; i++) {
			...
			VLOG(2) << row_format.c_str() << " " << i << " " << r_norm << " "
			          << s_norm << " " << primal_eps << " " << dual_eps;
...
#endif //SENSEMAP_OPTIM_SOLVER_L1_SOLVER_H_
