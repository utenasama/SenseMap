...
#ifndef SENSEMAP_UTIL_BITMAP_H_
#define SENSEMAP_UTIL_BITMAP_H_
...
public:
  ...
  BitmapColor<uint8_t> GetPixel(int x, int y) const;
  ...
  BitmapColor<float> InterpolateBilinear(double x, double y) const;

  // Extract EXIF information from bitmap. Returns false if no EXIF information
  // is embedded in the bitmap.
  bool HasDewarp() const;
  bool ExifFocalLength(double* focal_length) const;
  bool ExifIntrinsic(std::vector<double>& intrinsic) const;
  bool ExifLatitude(double* latitude) const;
  bool ExifLongitude(double* longitude) const;
  bool ExifAltitude(double* altitude) const;
  bool ExifRltAltitude(double* relative_altitude) const;
  bool ExifPitchDegree(double* pitch) const;
  bool ExifRollDegree(double* roll) const;
  bool ExifYawDegree(double* yaw) const;
  bool ExifRtkFlag(int8_t* rtk_flag) const;
  bool ExifRktStdLon(double* rtk_std_lon) const;
  bool ExifRktStdLat(double* rtk_std_lat) const;
  bool ExifRktStdHgt(double* rtk_std_hgt) const;
