...
BitmapColor<uint8_t> Bitmap::GetPixel(int x, int y) const {
...
BitmapColor<float> Bitmap::InterpolateBilinear(double x, double y) const {
    // Image border clipping
	x = std::max(x, 0.0);
	x = std::min(x, (double)(width_ - 1));
	y = std::max(y, 0.0);
	y = std::min(y, (double)(height_ - 1));
	...
	const int x1 = x0 + 1;
	int y0 = static_cast<int>(std::floor(inv_y));
	if (y0 == height_ - 1) {
	    y0--;
	}
	const int y1 = y0 + 1;
...
bool Bitmap::HasDewarp() const {
	return image_exif_.DewarpData.hasDewarp();
}

bool Bitmap::ExifFocalLength(double* focal_length) const {
	if (image_exif_.Calibration.hasFocalLength()) {
		*focal_length = image_exif_.Calibration.FocalLength;
		return true;
	} else {
...
bool Bitmap::ExifIntrinsic(std::vector<double>& intrinsic) const {
	if (image_exif_.DewarpData.hasDewarpData()) {
		intrinsic.push_back(image_exif_.DewarpData.FocalLengthX);
		intrinsic.push_back(image_exif_.DewarpData.FocalLengthY);
		double principal_x = image_exif_.DewarpData.OpticalCenterX;
		double principal_y = image_exif_.DewarpData.OpticalCenterY;
		if (!image_exif_.Calibration.hasOpticalCenterX()) {
			principal_x += width_ / 2;
		}
		if (!image_exif_.Calibration.hasOpticalCenterY()) {
			principal_y += height_ / 2;
		}
		intrinsic.push_back(principal_x);
		intrinsic.push_back(principal_y);
		intrinsic.push_back(image_exif_.DewarpData.k1);
		intrinsic.push_back(image_exif_.DewarpData.k2);
		intrinsic.push_back(image_exif_.DewarpData.p1);
		intrinsic.push_back(image_exif_.DewarpData.p2);
		intrinsic.push_back(image_exif_.DewarpData.k3);
		return true;
	}
	return false;
}
...
bool Bitmap::ExifLatitude(double* latitude) const {
	if (image_exif_.GeoLocation.hasLatLon()) {
		*latitude = image_exif_.GeoLocation.Latitude;
		return true;
	} else {
		std::string str;
...
bool Bitmap::ExifLongitude(double* longitude) const {
	if (image_exif_.GeoLocation.hasLatLon()) {
		*longitude = image_exif_.GeoLocation.Longitude;
		return true;
	} else {
		std::string str;
...
bool Bitmap::ExifAltitude(double* altitude) const {
	if (image_exif_.GeoLocation.hasAltitude()) {
		*altitude = image_exif_.GeoLocation.Altitude;
		return true;
	} else {
		std::string str;
...
bool Bitmap::ExifRltAltitude(double* relative_altitude) const {
	if (image_exif_.GeoLocation.hasAltitude()) {
		*relative_altitude = image_exif_.GeoLocation.RelativeAltitude;
		return true;
	} else {
		std::string str;
		if (ReadExifTag(FIMD_EXIF_GPS, "GPSRelativeAltitude", &str)) {
			const boost::regex regex(".*?([0-9.]+).*?/.*?([0-9.]+).*?");
			boost::cmatch result;
			if (boost::regex_search(str.c_str(), result, regex)) {
				*relative_altitude = std::stold(result[1]) / std::stold(result[2]);
				return true;
			}
		}
	}
...
bool Bitmap::ExifPitchDegree(double* pitch) const {
	if (image_exif_.GeoLocation.hasOrientation()) {
		*pitch = image_exif_.GeoLocation.PitchDegree;
		// *pitch = image_exif_.GeoLocation.FlightPitchDegree;
		return true;
	}
	return false;
}

bool Bitmap::ExifRollDegree(double* roll) const {
	if (image_exif_.GeoLocation.hasOrientation()) {
		*roll = image_exif_.GeoLocation.RollDegree;
		// *roll = image_exif_.GeoLocation.FlightRollDegree;
		return true;
	}
	return false;
}

bool Bitmap::ExifYawDegree(double* yaw) const {
	if (image_exif_.GeoLocation.hasOrientation()) {
		*yaw = image_exif_.GeoLocation.YawDegree;
		// *yaw = image_exif_.GeoLocation.FlightYawDegree;
		return true;
	}
	return false;
}

bool Bitmap::ExifRtkFlag(int8_t* rtk_flag) const {
	if (image_exif_.GeoLocation.RtkFlag < 0) {
		return false;
	}
	*rtk_flag = image_exif_.GeoLocation.RtkFlag;
	return true;
}

bool Bitmap::ExifRktStdLon(double* rtk_std_lon) const {
	if (image_exif_.GeoLocation.RtkStdLon != DBL_MAX) {
		*rtk_std_lon = image_exif_.GeoLocation.RtkStdLon;
		return true;
	}
	return false;
}

bool Bitmap::ExifRktStdLat(double* rtk_std_lat) const {
	if (image_exif_.GeoLocation.RtkStdLat != DBL_MAX) {
		*rtk_std_lat = image_exif_.GeoLocation.RtkStdLat;
		return true;
	}
	return false;
}

bool Bitmap::ExifRktStdHgt(double* rtk_std_hgt) const {
	if (image_exif_.GeoLocation.RtkStdLat != DBL_MAX) {
		*rtk_std_hgt = image_exif_.GeoLocation.RtkStdHgt;
		return true;
	}
	return false;
}
...
bool Bitmap::Read(const std::string& path, const bool as_rgb) {
	...
	image_exif_ = TinyEXIF::EXIFInfo(data.data(), length);
