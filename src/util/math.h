...
#ifndef SENSEMAP_UTIL_MATH_H_
#define SENSEMAP_UTIL_MATH_H_
...
#ifndef RAD2DEG
#define RAD2DEG(rad) rad / 0.0174532925199432
#endif
...
// Check str is double or not
inline bool IsDouble(const std::string& str);

// Check input is leap year or not
inline bool IsLeapYear(int year);

// Check year, mon, day is legal or not
inline bool IsDateLegal(int year, int mon, int day);

// Check hour, min, second is legal or not
inline bool IsTimeLegal(int hour, int min, int second);
...
bool IsDateLegal(int year, int mon, int day) {
...
bool IsTimeLegal(int hour, int min, int second) {
  if (hour < 0 || min < 0 || second < 0) return false;

  if (hour > 23 || min > 59 || second > 59) return false;
