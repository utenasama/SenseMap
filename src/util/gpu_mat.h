...
#ifndef SENSEMAP_UTIL_GPU_MAT_H_
#define SENSEMAP_UTIL_GPU_MAT_H_
...
class GpuMat {
 public:
  ...
  void FillWithScalar(const T value, const cudaStream_t stream);
...
GpuMat<T>::GpuMat(const size_t width, const size_t height, const size_t depth)
    : array_(nullptr),
      ...
      height_(height),
      depth_(depth) {
  cudaError_t error = cudaMallocPitch((void**)&array_ptr_, &pitch_,
                                 width_ * sizeof(T), height_ * depth_); 
  if (error != cudaSuccess) {
    std::cerr << "Cuda error in GpuMat(GpuMat)!" << std::endl;
    std::cout << "width = " << width_ << ", height = " << height_ << ", depth_ = " << depth_ << std::endl;
  }
...
template <typename T>
void GpuMat<T>::FillWithScalar(const T value, const cudaStream_t stream) {
  CUDA_SAFE_CALL(
      cudaMemsetAsync(array_ptr_, value, width_ * height_ * depth_ * sizeof(T), stream));
}
...
void GpuMat<T>::CopyToDevice(const T* data, const size_t pitch) {
  cudaError_t error = cudaMemcpy2D((void*)array_ptr_, (size_t)pitch_, 
                              (void*)data, pitch, width_ * sizeof(T), 
                              height_ * depth_, cudaMemcpyHostToDevice);
  if (error != cudaSuccess) {
    std::cerr << "Cuda error in GpuMat(CopyToDevice)!" << std::endl;
    std::cout << "width = " << width_ << ", height = " << height_ << ", depth_ = " << depth_ << ", pitch = " << pitch_ << std::endl;
  }
  CUDA_SAFE_CALL(error);
}
...
#endif  // SENSEMAP_UTIL_GPU_MAT_H_
