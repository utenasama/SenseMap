...
Eigen::Vector3d TriangulatePointHomogeneous(
  ...
  A.row(0) = point1(0) * proj_matrix1.row(2) - point1(2)* proj_matrix1.row(0);
  A.row(1) = point1(1) * proj_matrix1.row(2) - point1(2)* proj_matrix1.row(1);
  A.row(2) = point2(0) * proj_matrix2.row(2) - point2(2)* proj_matrix2.row(0);
  A.row(3) = point2(1) * proj_matrix2.row(2) - point2(2)* proj_matrix2.row(1);
...
Eigen::Vector3d TriangulatePoint(const Camera& camera1,
    ...
    if (camera1.ModelName().compare("SPHERICAL") == 0) {
        A.row(0) = hpoint1(0) * proj_matrix1.row(2) - hpoint1(2) * proj_matrix1.row(0);
        A.row(1) = hpoint1(1) * proj_matrix1.row(2) - hpoint1(2) * proj_matrix1.row(1);
    } else {
        A.row(0) = point1(0) * proj_matrix1.row(2) - proj_matrix1.row(0);
        A.row(1) = point1(1) * proj_matrix1.row(2) - proj_matrix1.row(1);
    }
    if (camera2.ModelName().compare("SPHERICAL") == 0) {
        A.row(2) = hpoint2(0) * proj_matrix2.row(2) - hpoint2(2) * proj_matrix2.row(0);
        A.row(3) = hpoint2(1) * proj_matrix2.row(2) - hpoint2(2) * proj_matrix2.row(1);
    } else {
        A.row(2) = point2(0) * proj_matrix2.row(2) - proj_matrix2.row(0);
        A.row(3) = point2(1) * proj_matrix2.row(2) - proj_matrix2.row(1);
    }
...
std::vector<Eigen::Vector3d> TriangulatePointsHomogeneous(
  ...
  for (size_t i = 0; i < points3D.size(); ++i) {
    points3D[i] =
        TriangulatePointHomogeneous(proj_matrix1, proj_matrix2, 
                                    points1[i], points2[i]);
  }
...
Eigen::Vector3d TriangulateMultiViewPointHomogeneous( //need test
    const std::vector<Eigen::Matrix3x4d>& proj_matrices,
...
Eigen::Vector3d TriangulateMultiViewPoint(
  ...
  CHECK_EQ(proj_matrices.size(), points.size());
  CHECK_EQ(proj_matrices.size(), hpoints.size());

  Eigen::Matrix4d A = Eigen::Matrix4d::Zero();

  for (size_t i = 0; i < points.size(); i++) {
    Eigen::Vector3d point;
    if (cameras.at(i).ModelName().compare("SPHERICAL") == 0) {
      point = hpoints[i].normalized();
    } else {
      point = points[i].homogeneous().normalized();
    }
    const Eigen::Matrix3x4d term =
        proj_matrices[i] - point * point.transpose() * proj_matrices[i];
    A += term.transpose() * term;
