...
void MultiplePoseFromHomographyMatrix(const Eigen::Matrix3d& H,
  ...
  std::vector<size_t> positive_point_count;
  points3D->clear();
  for (size_t i = 0; i < R_cmbs.size(); ++i) {
    std::vector<Eigen::Vector3d> points3D_cmb;
    CheckCheirality(R_cmbs[i], t_cmbs[i], points1, points2, &points3D_cmb);
    ...
    positive_point_count.push_back(points3D_cmb.size());
  }

  for(size_t i=0; i < R_cmbs.size(); ++i){
    if(static_cast<double>(positive_point_count[i])>
       static_cast<double>(points3D->size())*0.5){
      Rs.push_back(R_cmbs[i]);   
      ts.push_back(t_cmbs[i]);
    }
  }
}
