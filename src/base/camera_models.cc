...
void ComputeDistortionValueAndJacobian(const double* params,
	...
	BaseCameraModel<OpenCVFisheyeCameraModel>::IterativeUndistortion(params,  &u, &v);
	...
	if(jacobian){
		...
		if(r > std::numeric_limits<double>::epsilon()){		
			...
			const double theta7 = theta4 * theta3;
      		const double theta8 = theta4 * theta4;
			const double theta9 = theta8 * theta;
			
			const double thetad =
            	theta * (1.0 + k1 * theta2 + k2 * theta4 + k3 * theta6 + k4 * theta8);
			const double dtheta_du = u / ((1 + r * r) * r); // fixed
			const double dtheta_dv = v / ((1 + r * r) * r); // fixed
			const double dthetad_dtheta = 1 + 3 * k1 * theta2 + 5 * k2 * theta4 + 7 * k3 * theta6 + 9 * k4 * theta8; 

			const double dthetad_du = dthetad_dtheta * dtheta_du;
			const double dthetad_dv = dthetad_dtheta * dtheta_dv;
		
			const double dx_du =  (v*v)/r3 * thetad + (u/r)*dthetad_du;
			const double dx_dv = -(u*v)/r3 * thetad + (u/r)*dthetad_dv;

			const double dy_du = -(u*v)/r3 * thetad + (v/r)*dthetad_du;
 			const double dy_dv =  (u*u)/r3 * thetad + (v/r)*dthetad_dv;

			Eigen::Matrix2d Du;
			Du << dx_du, dx_dv,
				    dy_du, dy_dv;
			
			Eigen::Matrix<double,2,4> Dp;
			Dp << u/r * theta3, u/r * theta5, u/r * theta7, u/r * theta9,
				    v/r * theta3, v/r * theta5, v/r * theta7, v/r * theta9;

			Eigen::Matrix2d Ux = Du.inverse();
			Eigen::Matrix<double, 2, 4>Up = - Du.inverse() * Dp;

			for(int m = 0; m < 2; ++m){
				for(int n = 0; n < 4; ++n){
					jacobian[0][m*4 + n] = Up(m,n);
				}
			}
			for(int m = 0; m < 2; ++m){
				for(int n = 0; n < 2; ++n){
					jacobian[1][m*2 + n] = Ux(m,n);
				}
			}		
		}
		else{
			for(int m = 0; m < 2; ++m){
				for(int n = 0; n < 4; ++n){
					jacobian[0][m*4 + n] = 0.0;
				}
			}
			for(int m = 0; m < 2; ++m){
				for(int n = 0; n < 2; ++n){
					jacobian[1][m*2 + n] = 0.0;
				}
			}	
		}
	}
