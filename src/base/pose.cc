...
Eigen::Vector4d ConcatenateQuaternions(const Eigen::Vector4d& qvec1,
    ...
    const Eigen::Quaterniond quat1(normalized_qvec1(0), normalized_qvec1(1), normalized_qvec1(2), normalized_qvec1(3));
    const Eigen::Quaterniond quat2(normalized_qvec2(0), normalized_qvec2(1), normalized_qvec2(2), normalized_qvec2(3));
...
Eigen::Matrix3d RotationFromUnitVectors(const Eigen::Vector3d& vector1,
    ...
    // // Method 2. Rodrigues' formulation
    // return Eigen::Matrix3d::Identity() + v.norm() * v_x + (1 - c) * v_x * v_x;
}
...
bool CheckCheirality(const Eigen::Matrix3d& R, const Eigen::Vector3d& t,
    ...
    const double min_depth = std::numeric_limits<double>::epsilon();
...
bool CheckCheiralitySphericalCamera(const Eigen::Matrix3d& R, 
    ...
    const double max_depth = 1000.0f * (R.transpose() * t).norm();
    const double min_depth = -max_depth;
    ...
    for (size_t i = 0; i < points1.size(); ++i) {
        ...
        double angular_error1 = CalculateAngularErrorSphericalCamera(
                            points1[i],point3D,proj_matrix1);
        double angular_error2 = CalculateAngularErrorSphericalCamera(
                            points2[i],point3D,proj_matrix2);

        if(RadToDeg(angular_error1)>2.0||RadToDeg(angular_error2)>2.0){
            continue;
        }
